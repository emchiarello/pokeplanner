...
function LargeCard({ pokemon, inTeam }: { pokemon: Pokemon; inTeam?: boolean }) {
  return (
    <div
      className={`border-2 rounded-[13px] ${inTeam ? 'border-green-500 shadow-md shadow-green-500' : 'border-blue-100'}`}
    >
      <div
        className={`flex flex-col justify-center items-center gap-2 p-2 border rounded-xl ${inTeam ? 'border-green-500' : ''}`}
      >
...
function TypePairs({ pokemon, variant }: { pokemon: Pokemon; variant: 'default' | 'sm' }) {
  ...
  return (
    <div className="w-full flex gap-1">
      <TypeShell
        className={`${pokemon.typeTwo ? 'rounded-l-xl' : 'rounded-xl'} ${TYPE_TO_COLOR[pokemon.typeOne]}`}
      >
