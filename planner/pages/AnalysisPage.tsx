...
function TypeCard({ pokemonType, team }: { pokemonType: string; team: Pokemon[] }) {
  const layout =
    team.length < 3 ? 'flex justify-center gap-1' : 'grid grid-cols-3 md:grid-cols-6 gap-1';

  return (
    <div className="flex flex-col items-center border rounded-2xl p-2">
      <img
        src={`/assets/${pokemonType}_icon.png`}
        alt={pokemonType}
        className={`${TYPE_TO_COLOR[pokemonType]} rounded-full p-1 mb-2 hidden md:block`}
      />
      <p
        className={`w-full text-center rounded-lg px-2 py-1 mb-2 m-0 ${TYPE_TO_COLOR[pokemonType]} xl:bg-transparent`}
      >
...
function analyzeType(type: string, pokemon: Pokemon) {
  ...
  const analysisTypeTwo = pokemon.typeTwo
    ? typeVector[getPokemonTypes().indexOf(pokemon.typeTwo)]
    : 1;
