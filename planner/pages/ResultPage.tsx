...
export function ResultPage({
  ...
  useEffect(() => {
    ...
    Promise.all(
      pokedexes.map((dex) => fetch(`${BASE_URL}/pokedex/${dex}`).then((res) => res.json())),
    )
      ...
      {pokemonDetails &&
        pokemonDetails
          .filter(
            (pokemon) =>
              !typeFilter ||
              pokemon.typeOne === typeFilter ||
              (pokemon.typeTwo && pokemon.typeTwo === typeFilter),
          )
          .map((pokemon: Pokemon) => (
            <Clickable key={pokemon.name} onClick={() => togglePokemonInTeam(pokemon)}>
              <Card
                pokemon={pokemon}
                variant="lg"
                inTeam={team.filter((member) => member.name === pokemon.name).length > 0}
              />
            </Clickable>
          ))}
    </div>
